...
(function(Opal) {
  ...
  var $a, $b, TMP_1, self = Opal.top, $scope = Opal, nil = Opal.nil, $breaker = Opal.breaker, $slice = Opal.slice, $klass = Opal.klass, $range = Opal.range, apple_pie = nil, frame = nil;

  Opal.add_stubs(['$require', '$attr_reader', '$fill_with', '$private', '$new', '$eat', '$reduce', '$+', '$puts', '$filling']);
  self.$require("opal");
  (function($base, $super) {
    function $Pie(){};
    var self = $Pie = $klass($base, $super, 'Pie', $Pie);

    var def = self.$$proto, $scope = self.$$scope;

    self.$attr_reader("filling");

    Opal.defn(self, '$initialize', function(filling) {
      var self = this;

      self.$fill_with(filling);
      return self.eaten = false;
    });

    Opal.defn(self, '$eat', function() {
      var self = this;

      return self.eaten = true;
    });

    self.$private();

    return (Opal.defn(self, '$fill_with', function(filling) {
      var self = this;

      return self.filling = filling;
    }), nil) && 'fill_with';
  })($scope.base, null);
  apple_pie = $scope.get('Pie').$new("apple");
  apple_pie.$eat();
